...
@category mole-lib
@example "the config root directory" { dir }
...
export def "file" []: nothing -> string { [(dir) connections.yaml] | path join }

# Absolute path to the saved-query directory (`<dir>/queries`).
@category mole-lib
@example "the query directory path" { querydir }
export def "querydir" []: nothing -> string { [(dir) queries] | path join }
