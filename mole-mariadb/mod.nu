...
@example "render only the core tables as a Mermaid ER diagram" {
  mole-mariadb schema --full --only "users,orders,order_items" -c mariadb-local-dev | mole-mermaid er-schema
}
...
export def "schema" [
  ...
  --only: string@"mariadb-tables-csv"              # keep only these tables — comma-sep names/globs (all views)
  --exclude: string@"mariadb-tables-csv"           # drop these tables — comma-sep names/globs (all views)
  --host(-h): string                               # override host
  --port(-p): int                                  # override port
  --user(-u): string                               # override user
  --password(-P): string                           # override password
  ...
  let conf = (ma-conf $connection $host $port $user $password $database $set)
  let data = (sql schema-filter (ma-schema-load $conf --refresh=$refresh) --only (sql csv-split $only) --exclude (sql csv-split $exclude))
  if $full { return $data }
